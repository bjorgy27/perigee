...
pub fn sv_from_coe(coe:&ElSetMatrix) -> (Matrix6xX<f64>, Vec<f64>, f64, Vec<f64>){

        let time_ahead: f64 = 0.5;

        let t_end: f64 = now_jd() + time_ahead;
    ...
    for col in 0..coe.ncols() {
        ...
        let x_0 = Vector6::new(r.x, r.y, r.z, v.x, v.y, v.z);

        let jd_epoch: f64 = coe[(1, col)];
        let dt_seconds: f64 = (t_end - jd_epoch) * 86400.0;
        ...
        (Matrix6xX::from_columns(&states), jd_epochs, t_end, dts)
...
fn now_jd() -> f64 {
    ...
    let unix_seconds: f64 = now.timestamp() as f64 + now.timestamp_subsec_micros() as f64 * 1e-6;
